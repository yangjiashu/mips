...
public class StudentServiceImpl implements IStudentService {
    ...
    public AnswerStatus assemble(int student_id, int questionId, String filePath) {
        ...
        if (!runError) {
            ...
            for (Map<String, Integer> mem : status.getMem()) { // 获取内存运行结果
                ...
                if (mem.get(address) != memMap.get(address)) {
            ...
            for (Map<String, Integer> reg : status.getReg()) { // 获取寄存器运行结果
                ...
                if (reg.get(name) != regMap.get(name)) {
    ...
    private String init(int questionId, String filePath) {
    ...
    private String[] initInstruction(List<MipsMemory> memories, List<MipsRegister> registers, MipsStack stack) {
        ...
        if (!registers.isEmpty()) {
            ...
            for (MipsRegister register : registers) {
                ...
                strs.add(setReg(name, register.getValue()));
            }
        }
    ...
    // 生成初始化寄存器的命令
    private String setReg(String name, int value) {
        return "addi " + name + ", $zero, " + value;
    }
